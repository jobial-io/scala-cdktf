...
yum-config-manager --add-repo https://archive.fedoraproject.org/pub/archive/fedora/linux/updates/37/Everything/x86_64/
yum-config-manager --add-repo https://archive.fedoraproject.org/pub/archive/fedora/linux/updates/38/Everything/x86_64/
...
"""
  ...
  def addDockerContainerToSystemd(name: String, user: String = "root") =
    addBashScript(s"/usr/local/bin/docker_checkpoint_${name}", s"""

  docker stop ${name}
  docker rm ${name}
""") >>
      addBashScript(s"/usr/local/bin/docker_restore_${name}", s"""
status=\\$$(docker inspect --format="{{.State.Status}}" ${name})
[ "\\$$status" == running ] ||  ( docker start ${name} && echo restore unsuccessful, starting container )
"""
      ) >>
      addUserDataLines(s"""container_id=$$(docker inspect --format="{{.Id}}" ${name})""") >>
      addFile(s"${homeDir(user)}/.config/systemd/user/${name}.service", s"""
[Unit]
Description=Service for docker container ${name} generated by scala-cdktf
Wants=network-online.target
After=network-online.target
...
Restart=on-failure
TimeoutStopSec=70
ExecStart=/usr/local/bin/docker_restore_${name}
ExecStop=/usr/local/bin/docker_checkpoint_${name}
ExecStopPost=/usr/local/bin/docker_checkpoint_${name}
PIDFile=/run/containers/storage/overlay-containers/$$container_id/userdata/conmon.pid
Type=forking
StandardOutput=append:/var/log/${name}.log
StandardError=append:/var/log/${name}.log
...
"""
  ...
  def dockerEcrLogin(region: String, accountId: String) = addUserDataLines(
    s"aws ecr get-login-password --region $region | docker login --username AWS --password-stdin $accountId.dkr.ecr.$region.amazonaws.com"
  )
